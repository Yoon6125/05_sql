package com.connectiontest2.java;
...
//			ResultSet result = st.executeQuery("select * from board where b_title='자유글';"); // 전송 할 sql 문

//			java.sql.Statement stmt= con.createStatement();
//			while (result.next()) { // 결과를 하나씩 빼기. 더 이상 없으면(행 수가 끝나면) false 리턴됨.
//				String name = result.getString("b_id"); // p_name 필드(열) 의 데이터 꺼내기(1개 꺼낸거에서)
...
//			System.out.println(result);
			...
			// DB 에 데이터 업데이트
			int result = st.executeUpdate("UPDATE board SET b_id='cat44' WHERE b_no='28'");
			// DB 에 데이터 삭제
			java.sql.Statement stmt = con.createStatement();
			int delete_process = stmt.executeUpdate("DELETE FROM board WHERE b_no='28'");
